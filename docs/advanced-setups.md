...
* CAPMOX needs `PVEDataStoreAdmin` on a storage suitable for ISO images for cloud-init. Create a dedicated storage for this (you can use subdirectories in an existing network share for example).
  CAPMOX deterministically selects an enabled ISO-capable storage with enough available capacity, and retries relevant storage-inventory failures without terminalizing the Machine. It names each ISO from the immutable ProxmoxMachine UID and logical bootstrap digest and records the exact storage volume, size, and accepted upload task on the ProxmoxMachine before waiting for PVE. Bootstrap reconciliation resumes that exact recorded task and never dispatches a second upload while its outcome is pending. Dispatch is serialized per ProxmoxMachine under controller-runtime's single-active-leader invariant, with a durable owner/lease claim followed by a second optimistic-concurrency write immediately before POST. A dead owner's intent is reclaimed only after lease expiry, exact artifact absence, and authoritative absence of every active PVE `imgcopy` task; deletion uses the same ownership and quiescence boundary before releasing the Machine finalizer. Controller shutdown cancels the dispatch context, so a former leader cannot POST after ownership revalidation. PVE supplies no fencing token: the guarantee is one CAPMOX dispatch under that serialized, single-leader, cancellation-respecting boundary, not unconditional PVE-side exactly-once execution. An accepted upload that finishes after a controller restart or early VM removal remains owned and is removed. Ambiguous or nonterminal upload ownership fails closed rather than treating an empty storage scan as cleanup proof. Successful readiness cleanup removes the exact ISO and ownership tag before durably clearing the completed upload record.
