...
* CAPMOX needs `PVEDataStoreAdmin` on a storage suitable for ISO images for cloud-init. Create a dedicated storage for this (you can use subdirectories in an existing network share for example).
  CAPMOX deterministically selects an enabled ISO-capable storage with enough available capacity, and retries relevant storage-inventory or durable-record write failures without terminalizing the Machine. It names each ISO from the immutable ProxmoxMachine UID and logical bootstrap digest and records the exact node, storage volume, size, and accepted upload task on the ProxmoxMachine before waiting for PVE. Bootstrap reconciliation resumes that exact recorded task on its original node, including after VM migration, and never dispatches a second upload while its outcome is pending. If PVE has expired an accepted task from history, reconciliation advances only after exact artifact proof and authoritative absence of every active `imgcopy` task. Dispatch is serialized per ProxmoxMachine under controller-runtime's single-active-leader invariant, with a durable owner/lease claim followed by a second optimistic-concurrency write immediately before POST. A dead owner's intent is reclaimed only after lease expiry, exact artifact absence, and authoritative upload-task quiescence; deletion uses the same ownership boundary before releasing the Machine finalizer. The upload HTTP request carries the reconciliation context, while ambiguity proof always uses a separate bounded context, so controller shutdown prevents a former leader from continuing the POST but cannot erase proof of an already accepted request. PVE supplies no fencing token: the guarantee is one CAPMOX dispatch under that serialized, single-leader, cancellation-respecting boundary, not unconditional PVE-side exactly-once execution. An accepted upload that finishes after a controller restart or early VM removal remains owned and is removed. Ambiguous or nonterminal upload ownership fails closed rather than treating an empty storage scan as cleanup proof. Successful readiness cleanup validates unmount and tag-removal task completion, removes the exact ISO and ownership tag, and only then durably clears the completed upload record. During rolling upgrade, a tagged exact legacy `user-data-<VMID>.iso` mount is statefully unmounted and deleted before the content-addressed format is enforced; other legacy-looking or foreign media remain outside CAPMOX mutation authority.
